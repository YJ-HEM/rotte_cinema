...
namespace rotte_cinema.dao
{
	class MySqlManager
	{
		...
		{
			using (MySqlCommand command = new MySqlCommand(sql.ToUpper(), connection))
			{
				connection.Open();
		...
		public static DataTable AdapterRead(string sql)
		{
			...
			MySqlDataAdapter adapter = new MySqlDataAdapter(sql.ToUpper(), connection); /* 데이터 테이블 형식으로 반환. */
			adapter.Fill(dt);
			return dt;
		}
	}
